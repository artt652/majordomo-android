...
⚠️ **ВНИМАНИЕ: Тестовая сборка!**

Данная адаптация Majordomo под Termux/Android находится в стадии тестирования (alpha/experimental) и не предназначена для использования в реальных системах умного дома, отвечающих за безопасность, охрану, контроль доступа, газ/воду/электричество и другие критичные функции.

Совместимость PHP 8.5 с кодом, написанным под PHP 7.x, обеспечена точечными патчами, а не полным аудитом — возможны необнаруженные edge-case ошибки.

Redis работает через pure-PHP Predis-полифилл (redis_compat.php), а не через нативное расширение — поведение под нагрузкой может быть не идентично оригиналу.

Watchdog/cycle-процессы на Android могут завершаться системой (Doze, OOM killer) без уведомления.

Отсутствует root — часть стандартных механизмов мониторинга/перезапуска недоступна или заменена самописными скриптами.

Используйте на свой риск, делайте резервные копии БД и конфигурации, не подключайте к сборке устройства, отказ которых может привести к травмам, порче имущества или иному ущербу.
